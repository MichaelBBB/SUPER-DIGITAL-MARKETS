    ...
    <div className="min-h-screen bg-gray-900 text-white p-8 font-mono">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
          ...
          <p className="text-3xl font-bold text-green-400">{totalOrders.toLocaleString()}</p>
        </div>
        <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
          <h3 className="text-lg font-bold text-gray-300 mb-2">Total Revenue</h3>
          <p className="text-3xl font-bold text-cyan-400">${totalRevenue.toLocaleString()}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {rawData.map((row: any, idx) => (
          <div key={idx} className="bg-gray-800 p-6 rounded-lg border border-gray-600 hover:border-cyan-500 transition-colors">
            <h3 className="text-xl font-bold capitalize text-cyan-300 mb-2">{row.region}</h3>
            <div className="space-y-2">
              <p className="text-sm text-gray-400">Count: <span className="text-white font-bold">{row.count.toLocaleString()}</span></p>
              <p className="text-2xl font-bold text-green-400">${(row.count * 5).toLocaleString()}</p>
