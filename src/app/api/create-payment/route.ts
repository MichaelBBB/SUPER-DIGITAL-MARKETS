...
export async function POST(request: Request) {
  try {
    ...
    console.log("🔍 RAW PEACH RESPONSE STATUS:", res.status);
    console.log("🔍 RAW PEACH RESPONSE BODY:", text);

    let data;
    try { 
      data = JSON.parse(text); 
      console.log("🔍 PARSED PEACH RESPONSE:", JSON.stringify(data, null, 2));
    } catch (e) { 
    ...
    // Check for redirectUrl OR id (Peach sometimes returns just an ID)
    if (data.redirectUrl) {
      ...
      console.error("❓ UNKNOWN RESPONSE FORMAT (No URL or ID):", data);
