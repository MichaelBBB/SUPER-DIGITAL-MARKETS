  ...
  const handlePeachPayment = async () => {
    ...
    try {
      const res = await fetch('/api/create-payment', {
        ...
        body: JSON.stringify({ 
          amount, 
          currency: 'ZAR', // ✅ CHANGED TO ZAR AS REQUIRED BY PEACH LIVE ACCOUNT
      ...
      if (!res.ok) {
        if (data.fallback === 'whatsapp') {
          if (confirm("Peach Payments is currently unavailable for this order. Would you like to complete your order via WhatsApp instead?")) {
            handleWhatsAppOrder();
          }
        } else {
      ...
      alert(`Peach Payments Error:\n\n${error.message}\n\nPlease use Manual Transfer or WhatsApp instead.`);
    } finally {
      setLoading(false);
    }
  };
